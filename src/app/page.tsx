  ...
  return (
    <main className="min-h-screen bg-black text-white">
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-zinc-800 bg-zinc-950 p-4 shadow-2xl">
              ...
              <div className="mt-4 flex items-end justify-between gap-4">
                <div>
                  <p className="text-xs text-zinc-400">Kişiye özel hazırlık</p>
                  <h2 className="text-xl font-black sm:text-2xl">MühürZen Bakır Mühür Bilekliği</h2>
                  <p className="mt-1 text-xs text-zinc-500">İsim • Anne adı • Doğum tarihi • Niyet</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-zinc-400 line-through">₺1600</p>
      ...
      <section id="test" className="mx-auto max-w-7xl px-5 py-16">
        <div className="rounded-[2rem] border border-zinc-800 bg-zinc-950 p-6 md:p-10">
          ...
          <div className="mt-6 rounded-3xl border border-amber-500/30 bg-amber-500/10 p-6">
            ...
            <a href="#siparis" className="mt-5 inline-flex rounded-full bg-amber-500 px-6 py-3 font-black text-black hover:bg-amber-400">Bu Niyetle Sepete Ekle</a>
          </div>
        </div>
      </section>
      ...
      <section id="siparis" className="mx-auto max-w-7xl px-5 py-16">
        <div className="rounded-[2rem] border border-zinc-800 bg-zinc-950 p-6 md:p-10">
          <p className="text-sm font-bold uppercase tracking-[0.25em] text-amber-400">Sepete Ekle</p>
          <h2 className="mt-3 text-3xl font-black sm:text-4xl">Sipariş notunuzu ekleyin, ödeme adımına geçin.</h2>
          <p className="mt-4 max-w-3xl leading-7 text-zinc-400">Sipariş notu alanına isim, anne adı, doğum yılı ve niyetinizi yazabilirsiniz.</p>

          {step === "form" && (
            <>
              <div className="mt-8 grid gap-3 rounded-3xl border border-amber-500/20 bg-black p-4 text-sm text-zinc-300 sm:grid-cols-3">
                <div><b className="text-amber-400">1.</b> Sipariş notunu yaz</div>
                <div><b className="text-amber-400">2.</b> Sepete ekle</div>
              ...
              <textarea value={form.note} onChange={(e) => update("note", e.target.value)} placeholder="Sipariş notu: İsim, anne adı, doğum yılı ve niyetinizi yazın. Örn: Ahmet / Anne adı: Ayşe / Doğum yılı: 1990 / Niyet: ilişki uyumu..." className="mt-4 min-h-32 w-full rounded-2xl border border-zinc-800 bg-black px-5 py-4 outline-none focus:border-amber-500" />
              ...
              <button disabled={loading} onClick={createOrder} className="mt-6 inline-flex w-full justify-center rounded-full bg-amber-500 px-8 py-4 text-lg font-black text-black hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-60">
                {loading ? "Sepete ekleniyor..." : "Sepete Ekle ve Ödemeye Geç - ₺999"}
              </button>
              <a
                href={whatsappUrl}
                target="_blank"
          ...
          {step === "payment" && (
            <div id="odeme" className="mt-8 rounded-[2rem] border border-amber-500/30 bg-amber-500/10 p-6">
              <p className="text-sm font-bold uppercase tracking-[0.2em] text-amber-400">Ödeme Adımı</p>
              <h3 className="mt-3 text-3xl font-black">Sipariş sepetine eklendi.</h3>
              ...
              <p className="mt-5 text-sm leading-6 text-zinc-300">Ödeme açıklamasına sipariş numaranı yaz. Ödemeden sonra aşağıdaki butona bas; siparişin ödeme kontrol listesine düşer ve Telegram bildirimi alınır.</p>
              {error && <div className="mt-5 rounded-2xl border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-100">{error}</div>}
              <button disabled={loading || receiptLoading} onClick={notifyPaid} className="mt-6 w-full rounded-full bg-amber-500 px-8 py-4 text-lg font-black text-black hover:bg-amber-400 disabled:opacity-60">
                {loading || receiptLoading ? "İşlem alınıyor..." : "Ödemeyi Bildir"}
              </button>
            </div>
          )}
